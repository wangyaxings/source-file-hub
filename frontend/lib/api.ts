...
class ApiClient {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error instanceof TypeError && error.message.includes("fetch")) {
          throw new Error(
            "Network connection failed, please check server status"
          );
        }
      } else {
  ...
  async login(data: LoginRequest): Promise<LoginResponse> {
    ...
    if (response.status === 307) {
      ...
      if (
        ...
        result.location.includes("/2fa/totp/validate")
      ) {
        throw new Error(
          "2FA_VERIFICATION_REQUIRED: Please enter your 2FA verification code"
        );
      }
  ...
  async uploadFile(
    ...
    if (!response.ok) {
      ...
      err.details = errorData?.details;
      err.request_id =
        response.headers.get("X-Request-ID") || errorData?.details?.request_id;
      err.status = response.status;
      throw err;
    }
  ...
  async getFileVersions(type: string, filename: string): Promise<FileInfo[]> {
    const response = await this.request<{
      versions: FileInfo[];
      count: number;
    }>(`/files/versions/${type}/${filename}`);
    return response.data?.versions || [];
  }

  async downloadFile(path: string): Promise<void> {
    // Remove downloads/ prefix if present since the API expects the path relative to downloads
    const cleanPath = path.startsWith("downloads/")
      ? path.substring("downloads/".length)
      : path;
    ...
    let filename = path.split("/").pop() || "download"; // fallback to path-based name
  ...
  async restoreFile(fileId: string): Promise<void> {
    ...
    if (!response.ok) {
      ...
      const errorData = await response
        ...
        .catch(() => ({ error: response.statusText }));
      throw new Error(
        errorData.error || `Restore failed: ${response.statusText}`
      );
    }
  ...
  async clearRecycleBin(): Promise<void> {
    ...
    if (!response.ok) {
      ...
      const errorData = await response
        ...
        .catch(() => ({ error: response.statusText }));
      throw new Error(
        errorData.error || `Clear recycle bin failed: ${response.statusText}`
      );
    }
  ...
  async adminDisable2FA(userId: string): Promise<void> {
    const resp = await this.request(
      `/admin/users/${encodeURIComponent(userId)}/2fa/disable`,
      { method: "POST" }
    );
  ...
  async adminEnable2FA(userId: string): Promise<void> {
    const resp = await this.request(
      `/admin/users/${encodeURIComponent(userId)}/2fa/enable`,
      { method: "POST" }
    );
  ...
  async adminResetPassword(
    userId: string
  ): Promise<{ username: string; temporary_password: string }> {
    const resp = await this.request<{
      username: string;
  ...
  ): Promise<{ items: any[]; count: number; page: number; limit: number }> {
    const qs = new URLSearchParams();
  ...
  async getVersionManifestWeb(
    type: "roadmap" | "recommendation",
    versionId: string
  ): Promise<any> {
    const url = `${this.baseUrl}/versions/${type}/${encodeURIComponent(
      versionId
    )}/manifest`;
    const response = await fetch(url, {
      credentials: "include", // Important: Include credentials for session cookies
  ...
  async updateVersionTagsWeb(
    type: "roadmap" | "recommendation",
    versionId: string,
    tags: string[]
  ): Promise<void> {
    await this.request(
      `/versions/${type}/${encodeURIComponent(versionId)}/tags`,
      {
        method: "PUT",
  ...
  async setupTOTP(): Promise<{ secret: string; otpauth_url: string }> {
    ...
    const setupResp = await this.request<any>("/auth/ab/2fa/totp/setup", {
      method: "POST",
    });
    ...
    if (!secret) {
      const confirm = await this.request<any>("/auth/ab/2fa/totp/confirm", {
        method: "GET",
      });
    ...
    const otpauth_url = `otpauth://totp/${encodeURIComponent(
      issuer
    )}:${encodeURIComponent(label)}?issuer=${encodeURIComponent(
      issuer
